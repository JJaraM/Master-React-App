export default {
  name: 'light',
  colorA: '#212841',
  colorB: '#F4F7FC',
  colorC: '#FFF',

  'primary-color': '#212841',
  'primary-color-hover': '#FFF',

  'form-control-background': {
    value: '#F7F7F7',
    description: 'Background use for the form controls',
  },

  'form-control-border': '1px solid #e9eaee',
  'form-control-color': '#000',

  'top-menu-background': '#FFF',
  'top-menu-collapse-color': '#000',

  'side-menu-background': '#212841',
  'side-menu-color': '#FFF',
  'side-menu-hover-background': '#1a1f33',
  'side-menu-subMenu-background': '#1a1f33',

  // Side Menu Icon
  'side-menu-icon-background': 'rgba(255,255,255,.22)',
  'side-menu-icon-background-hover': '#43b581',

  'side-menu-1-background': '#28304e',
  'side-menu-1-background-hover': '#1a2034',

  'side-menu-2-background': '#2e385b',
  'side-menu-2-background-hover': '#292B2F',

  'body-background': '#F4F7FC',

  'subcard-background': '#F7F7F7',
  'subcard-color': '#000',
  'subcard-box-shadow': '0 3px 8px 0 rgba(27,41,85,.05)',
  'subcard-border': '1px solid #E9EAEE',

  'table-background': '#FFF',
  'table-hover': '#F9FBFD',
  'tr-background': '#FFF',
  'tr-background-hover': '#E3E3E3',
  'table-button-background': '#3CA374',
  'table-th-background': '#f9fbfd',
  'table-th-color': '#95aac9',
  'table-th-top-border': '1px solid #edf2f9',
  'table-th-bottom-border': '1px solid #edf2f9',
  'table-td-border-top': '1px solid #edf2f9',
  'global-color': '#000',

  'card-background': '#FFF',
  'card-border': 'rgba(47,49,54,.9)',
  'card-box-shadow': '0 25px 50px rgba(8,21,66,.06)',
  'card-box-shadow-hover': '0 2px 4px rgba(126,142,177,.12)',
  'card-header-background': '#FFF',
  'card-header-title': '#000',
  'card-h1-color': 'rgba(255,255,255,.7)',
  'card-p-color': '#FFF',
  'card-hyperlink': '#000',

  'section-background': '#28304E',
  'section-h5': '#3e4676',
};
